...
class SyscallTracer(Application):
    ...
    def __init__(self):
        ...
        #self.channel.exchange_declare(exchange='syscalls', exchange_type='fanout')
    ...
    # TODO change this to only be data we care about
    def displaySyscall(self, syscall):
        text = syscall.format()
        if syscall.result is not None:
            text = "%s,%s" % (text, syscall.result)
        
        #embed() #pop an IPython shell

        #self.channel.basic_publish(exchange='syscalls', routing_key='', body=text)
        error(text)
